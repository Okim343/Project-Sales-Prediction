"""
Test version of the daily update pipeline for continuous learning.
...
def process_mlb_batch_with_validation_limited(
    ...
    """
    Limited version of MLB batch processing for testing.
    ...
    logger.info(f"Limiting batch processing to {max_mlbs} MLBs for testing")

    for mlb in batch_mlbs:
        if processed_count >= max_mlbs:
            logger.info(
                f"Reached maximum MLB test limit ({max_mlbs}), stopping processing"
            )
            break

        try:
            logger.info(f"Processing MLB {mlb} ({processed_count + 1}/{max_mlbs})...")
            ...
            try:
                ...
                if len(X_val) > 0:
                    ...
                    if (
                        ...
                        forecast_result = generate_forecast_for_mlb_test(
    ...
    logger.info(
        f"Completed limited batch processing: {len(batch_results['models'])} models updated out of {processed_count} processed"
    )
    return batch_results


def generate_forecast_for_mlb_test(
    mlb: str, model, mlb_data: pd.DataFrame
) -> Optional[Tuple]:
    """
    Generate forecast for a single MLB using its updated model (test version).
    ...
    try:
        ...
        last_features = np.array(mlb_data.iloc[-1][FEATURES].values).reshape(1, -1)
        ...
        logger.error(f"Failed to generate test forecast for MLB {mlb}: {e}")
        return None


def main():
    """Main execution function for the test daily update pipeline."""
    ...
    try:
        logger.info("=== TEST DAILY UPDATE PIPELINE STARTED ===")
        ...
        logger.info("Step 1: Checking last successful test run date...")
        last_run = get_last_successful_run(run_type="test_daily")

        if last_run:
            since_date = last_run["run_timestamp"].date()
            logger.info(f"Last successful test daily run: {since_date}")
        else:
            # If no previous test daily runs, look for any test run as fallback
            last_run = get_last_successful_run(run_type="test_incremental")
            if last_run:
                since_date = last_run["run_timestamp"].date()
                logger.info(
                    f"No test daily runs found, using last test run: {since_date}"
                )
            else:
                # Default to 7 days ago if no runs found
                since_date = (datetime.now() - timedelta(days=7)).date()
                logger.info(
                    f"No previous test runs found, defaulting to 7 days ago: {since_date}"
                )
        ...
        if new_data.empty:
            logger.info(
                f"No new data found since {since_date}. Test pipeline completed with no updates."
            )
        ...
        # Step 4: Load existing test models
        logger.info("Step 4: Loading existing test models...")
        test_models_file = (
            AppConfig.MLB_REGRESSORS_FILE.parent / "test_mlb_regressors.pkl"
        )

        if not test_models_file.exists():
            logger.info(
                "No existing test models found, running limited full training first..."
            )
            # Run limited full training to create test models
            clean_data = process_sales_data(new_data)
            feature_data = create_time_series_features(clean_data)

            logger.info(
                f"Training limited models (up to {AppConfig.TEST_MLB_COUNT} MLBs) from scratch..."
            )
            mlb_forecast, original_models = forecast_future_sales_direct_limited(
                feature_data, AppConfig.FORECAST_DAYS_LONG, AppConfig.TEST_MLB_COUNT
            )

            # Save the test models
            save_models(original_models, test_models_file)
            logger.info(f"Created and saved {len(original_models)} test models")
        else:
            try:
                original_models = load_models(test_models_file)
        ...
        # Step 4.5: Backup existing test models before updating
        logger.info("Step 4.5: Backing up existing test models...")
        try:
        ...
        # Step 5: Process each MLB with validation and rollback mechanism (LIMITED)
        logger.info(
            f"Step 5: Processing up to {AppConfig.TEST_MLB_COUNT} MLBs with incremental updates and validation..."
        )
        ...
        mlbs_to_update = list(mlbs_with_new_data.intersection(mlbs_with_models))

        logger.info(f"Found {len(mlbs_with_new_data)} MLBs with new data")
        logger.info(f"Found {len(mlbs_with_models)} MLBs with existing test models")
        logger.info(
            f"Will attempt to update up to {min(len(mlbs_to_update), AppConfig.TEST_MLB_COUNT)} MLBs"
        )

        updated_models = {}
        final_forecasts = {}
        rollback_stats["total_processed"] = min(
            len(mlbs_to_update), AppConfig.TEST_MLB_COUNT
        )

        if mlbs_to_update:
            # Process limited number of MLBs
            ...
            # Memory cleanup
            gc.collect()
            logger.debug("Test processing completed, memory cleaned up")
        else:
            logger.info(
                "No MLBs to update (no intersection between new data and existing test models)"
            )

        models_updated = len(updated_models)

        # Log rollback statistics
        logger.info("=== TEST ROLLBACK STATISTICS ===")
        ...
        if updated_models:
            ...
            try:
                ...
                if not is_consistent or consistency_issues:
                    ...
                    logger.info("All test models passed consistency validation")
        ...
        if final_forecasts:
            logger.info(f"Step 6: Validating {len(final_forecasts)} test forecasts...")
            try:
        ...
        # Step 6.5: Save forecasts to TEST SQL database table
        if final_forecasts:
            logger.info(
                f"Step 6.5: Saving {len(final_forecasts)} validated test forecasts to SQL database ({DatabaseConfig.TEST_FORECAST_TABLE})..."
            )
            try:
                db_manager.save_forecasts_to_sql(
                    final_forecasts, DatabaseConfig.TEST_FORECAST_TABLE
                )
                logger.info("Test forecasts saved successfully!")
            except Exception as e:
                logger.error(f"Failed to save test forecasts: {e}")
                # Don't fail the entire pipeline for forecast saving issues
        else:
            logger.info(
                "No test forecasts to save (no models were updated successfully or all failed validation)"
            )

        # Step 7: Save updated test models (only models that improved or maintained performance)
        if updated_models:
            logger.info(f"Step 7: Saving {len(updated_models)} updated test models...")
            try:
                save_models(updated_models, test_models_file)
                logger.info(
                    f"Saved {len(updated_models)} updated test models to {test_models_file}"
                )
            except Exception as e:
            ...
            logger.info(
                "No updated test models to save (all models were rolled back or failed)"
            )

        # Save test forecast data to pickle
        if final_forecasts:
            test_forecast_file = (
                AppConfig.MLB_FORECAST_FILE.parent / "test_daily_forecast.pkl"
            )
            import pickle
        ...
        logger.info(
            "Test daily update pipeline - MLB processing completed successfully"
        )
        ...
        logger.info("TEST DAILY UPDATE PIPELINE COMPLETED SUCCESSFULLY!")
        ...
        logger.info(
            f"Processed {len(updated_models)} MLBs out of {AppConfig.TEST_MLB_COUNT} requested"
        )
        ...
        error_message = str(e)
        logger.error(f"Test daily update pipeline failed with error: {error_message}")
...
def main_with_specific_date(since_date: str):
    """
    Alternative test main function that uses a specific date for data import.

    Args:
        since_date: Date string in YYYY-MM-DD format to import data since this date
    """
    start_time = time.time()
    run_type = "test_daily_specific"
    records_processed = 0
    models_updated = 0
    error_message = None

    try:
        logger.info(f"=== TEST DAILY UPDATE PIPELINE STARTED (since {since_date}) ===")
        ...
        logger.info(f"Importing test data since {since_date}...")
        try:
            new_data = import_data_from_sql_since_date(
                user=DatabaseConfig.USER,
                password=DatabaseConfig.PASSWORD,
            ...
            records_processed = len(new_data)
            logger.info(
                f"Imported {records_processed:,} records since {since_date} for testing"
            )
        except Exception as e:
            error_message = f"Failed to import test data since {since_date}: {e}"
            logger.error(error_message)
            raise
        ...
        # Initialize rollback stats for this specific date run
        rollback_stats = {
            "models_improved": 0,
            "models_maintained": 0,
            "models_rolled_back": 0,
        ...
        error_message = str(e)
        logger.error(
            f"Test daily update pipeline with specific date failed: {error_message}"
        )
        end_time = time.time()
        execution_time = end_time - start_time
        log_pipeline_run(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        logger.info(
            f"Running test daily update pipeline with specific date: {specific_date}"
        )
        main_with_specific_date(specific_date)
    else:
        logger.info(
            f"Running test daily update pipeline (limited to {AppConfig.TEST_MLB_COUNT} MLBs)"
        )
        main()
